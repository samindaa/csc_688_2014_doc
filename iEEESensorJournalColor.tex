...
\title{Semi-Automatic Extraction of Training Examples from Sensor Readings for \hl{Fall Detection and Posture Monitoring}}
...
\begin{abstract}
While humans perform \hl{daily life activities accidents such as a fall or a collision} may occur. \hl{As
this might cause physical injuries}, an immediate identification of \hl{an accident could alert humans for a fast
response}. Currently, a plethora of inexpensive wireless motion sensing devices are available or can be assembled using off-the-shelf components. They \hl{can be} used to collect motion data \hl{in 
order} to monitor non-fall activities and \hl{to detect} fall events. However, there are remaining problems such as
automatic extraction of motion features that distinguishes among the prior categories. We \hl{propose}
 a framework to setup a wireless sensor network for collecting motion data. We also 
\hl{introduce a novel} method for semi-automatically extracting training examples from the motion data.
We developed a two-level classification network to accurately \hl{identify several types of}
non-fall activities and fall events. The proposed classification-network is a combination of neural
and Softmax regression networks.  Our empirical evaluations of the proposed two-layer network show that seven types of non-fall activities and four types of fall events are classified on our datasets with 100\% accuracy.  \end{abstract}
...
combination of them to collect motion datasets at  25 Hz to 200 Hz.  \hl{A comprehensive and recent review of wearable sensors for human activity monitoring can be found in} \cite{MukReview15}.

\par
Earlier studies demonstrated feasibility of various miniature wired or wireless
devices, and recent studies have focused on \hl{extracting knowledge from data of
...
assess fall-risks, \item monitor ADLs, and \item identify fall events \hl{has
...
features as input and  \hl{correctly classifies the four fall events}.
...
We attached wearable sensing devices to collect motion data and \hl{used software}
...
In our study, \hl{both fall and non-fall} activities of biped robots were identified with
100\%  accuracy after using a Kalman filter \cite{Welch:1995:IKF:897831} with
appropriate thresholding. \hl{However}, the same method would not be as successful for our human
...
examples, and \item to monitor and predict events. \end{inparaenum} \hl{This
...
\hl{Based on the platforms used, modern} activity detection methods can be broadly
...
\par \hl{Ojetola et al.} \cite{ojetolaFallDetection2011} \hl{report a method for detecting
...
\hl{The method  in}\cite{baekFallDetection2013} \hl{uses  data from} tri-axial
accelerometer  and  gyroscope  sensors, embedded in a necklace, for classifying
the  behavior  and  posture  of  human subjects. \hl{Their method distinguishes
...
\hl{A system  to detect  events that may cause trauma and disabilities  was
proposed in} \cite{leoneFallDetection2013}. \hl{It used readings from MEMS
accelerometer embedded in a wearable wireless device.} They use SVM for
the classification of different events. \hl{Bao and Intille} 
...
right ankle, and the right hip  of the subjects for collecting motion data. \hl{They used motion data for 
recognizing 20 activities, including walking, riding elevator, strength training, and
bicycle.} \hl{They evaluated the performance }of a decision table,
...
\hl{A fall detection, avoidance, and damage reduction mechanism for biped humanoid
robots was proposed in} {~\cite{Moya2015}}.  \hl{The authors
...
smart-phone,  \hl{five human actions -- walking, running, standing up, sitting
...
fall activity when a predetermined set of conditions were satisfied. \hl{However},
the method did not provide any prediction or hint that a fall could occur in
the near future. 

\hl{It was show in} \cite{steidlFallDetection2012}, that the sensors in the
smart-phones from different manufactures record significantly different range of
values for identical tasks.  \hl{To overcome this problem,} they trained a SVM with
...
chair.\hl{ Their method detected fall events with an accuracy of 84.8\%.} 

\hl{Methods that detect both simple and complex activities using readings from
...
walking, and the phone-not-on-the-person. \hl{Complex activities include}
...
features extracted from the datasets. \hl{They obtained 93\% accuracy} for simple
activities (with MLP), but only 50\% accuracy for complex activities.    

\hl{In} \cite{ShenFallDetectionPhone2015}, high-level fuzzy Petri-net based systems
...
\par \hl{Only three studies} have been reported for biped robots
...
other hand, our research goal is \hl{to identify  fall and non-fall events  after
they occur.}
...
tested our framework on multiple \hl{micro-controllers and on booster packs}.  Our
...
\centering
  ...
  \includegraphics[width=0.9\columnwidth,height = 1.5in]{plots/human_walk-crop.eps}}}
...
from each other and all 7 non-fall events. Although Fig.~\ref{fig:automatic_annotation3} does show as good separations for non-fall events, the results, reported in Section~\ref{Evaluation}, \hl{show that the separations are for good enough for classification of the events with 100\%
accuracy.}
...
\section*{Acknowledgement} \label{sec:Acknowledgement} \hl{Authors greatly
...
\bibliography{referencescolor,fallRisk}
