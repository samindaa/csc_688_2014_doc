...
\title{\hl{Semi-Automatic Extraction of Training Examples from Sensor Readings for Tracking Human Activities}}
...
such as a fall may occur. This might cause injuries to the human body, and immediate identification of it can alert for a fast response.  Currently, a plethora of inexpensive wireless motion sensing devices are available or can be assembled using off-the-shelf components. They are being used to collect motion data to monitor non-fall activities and fall events. \hl{However, there are remaining} problems such as automatic extraction of motion features that distinguishes among the prior categories. We have developed a framework to setup a wireless sensor network \hl{collecting motion data}. We also have introduced a novel method for semi-automatically extracting training examples from the motion data. \hl{We have developed} a two-level classification network to accurately identify each type of non-fall activities and fall events. The proposed classification-network is a combination of neural and softmax regression networks.  \hl{Our empirical evaluations of the proposed two-layer network show that seven types of non-fall activities and four types of fall events are classified on our datasets with 100\% accuracy.}
\end{abstract}
...
As reported in the \hl{Centers for Disease Control and Prevention} (CDC) website 
\cite{CDC2014July},
...
\caption{(a) h1{a wireless sensor device (assembled from a Tiva C Series  TM4C123G 
...
\hl{We have assembled a wireless sensing-device with a Tiva C Series TM4C123G microcontroller board, and three 
boosterpacks --- a
...
\begin{inparaenum}[($i$)] \item  \hl{to setup the WSN}, \item \hl{to collect data}, \item \hl{to extract training examples from sensor readings} (see Section \ref{sec:SemiAutomaticExtractionOfTrainingVectors}), \item \hl{to learn from sample examples}, and \item 
\hl{to monitor and predict events}. \end{inparaenum} This framework is general enough for other practitioners to use the
...
\hl{Modern activity detection methods} can be broadly categorized into two groups based on: 
...
\hl{Another detection method reported} in\cite{baekFallDetection2013} used necklace-shaped tri-axial
...
decision tree, and na\"{i}ve Bayes classifiers, where the decision tree \hl{variant showed} the best performance 
...
\hl{Our development framework, $\mu$Energia (\textit{pronounced as}: ``micro--Energia'') is available from the following web} 
\textit{site}:
...
\hl{We have divided each task into two components for an easier description and for the clarity of understanding: a} {\em module} \hl{ one or more} {\em representations}. \hl{A module implements executable code, while a representation exchange information from one module to another. Thus, a module may generate information for multiple representations as well as it may get inputs from multiple representations. }
\par
...
\hl{The directed graph in Fig.}~\ref{fig:framework} \hl{shows a framework generated task-schedule 
for  a Tiva C Series TM4C123G microcontroller to collect and process data from} 
\begin{inparaenum}[($i$)] \item \hl{Bosch Sensortec BMP180 pressure sensor}, \item \hl{Intersil 
ISL29023 ambient and infrared light sensor}, \item \hl{Sensirion SHT21 humidity and ambient 
temperature sensor},  \item  \hl{TMP006 non-contact infrared temperature sensor, and} \item 
\hl{InvenSense MPU-9150 --- a 9-axis MEMS motion sensor}. \end{inparaenum}
...
In the next section, we propose a method for \hl{the} semi-automatic extraction of training examples from motion datasets. These examples are used to teach a device/system that identifies desired events. 

\section{Semi-Automatic Extraction of  Training Examples}
\label{sec:SemiAutomaticExtractionOfTrainingVectors}

This section and the next section provide our main contributions of the paper. \hl{Here, we provide} a description 
...
\hl{Fall Right} &\textbf{FR}  \\ \hline \hline
...
our input data vectors  are in $\mathbb{R}^6$ and \hl{they consist of accelerometer and 
gyroscope values. The plots of the raw motion datasets for fall forward and  walk forward 
events are shown in Figures}~\ref{fig:human_fall_forward_crop} and 
\ref{fig:human_walk_forward-crop}, \hl{respectively. They demonstrate the complex nature of 
...
\centering
  ...
  \includegraphics[width=0.9\columnwidth,height = 1.5in]{plots/human_falling-crop.eps}}}
  \caption{\hl{Fall and walk forward motion datasets. The accelerometer ($\dot{x}$, 
...
\begin{figure}[htb]
	\centering
		\includegraphics[width = 0.74\columnwidth]{figures/FlowChartAlgoForTrainingExamples.pdf}
	\caption{Flow chart of an algorithm for semi-automatic extraction of \hl{training 
	examples}}
...
have posed. \hl{We learn the classifiers offline from the training example extracted 
according to the methods presented in Section} 
\ref{sec:SemiAutomaticExtractionOfTrainingVectors}. \hl{This section explains the classifiers 
that we have considered in our experiments and their configurations.}

\hl{Using our semi-automated training example extraction method, we have created $m \in 
\mathbb{N}$ 
training examples}
...
$\mathbf{y} \in 
\{0,1\}^k$, where  $k \in 
\mathbb{N}$ \hl{is the number of dichotomies. Each training example belongs only to one class 
and $\mathbf{y}$ uses the 1--of--$k$ coding scheme.}   

\subsection{\hl{Softmax Regression Algorithm}}
...
\hl{In this section we report the performance of our proposed algorithms and methods. A detailed description of the data collection and preprocessing steps can be found in Section} \ref{subsec:preDataCollection}.
%First we brieflydescribe our data collection procedure. 
We have experimented with one-layer and two-layer classifiers.  First, \hl{the performance} of one-layer softmax and neural networks classifiers are reported. Then that for two-layer networks are presented. 
...
\par
Because of failure of one-layer networks, we explored two-layer classification networks: \hl{the} first layer classified an input into two categories, a fall event or a non-fall event. Then, a second layer  differentiated events within its own category. A block diagram of the classifier  is shown in Fig.~\ref{fig:TrainedIdentificationModule}.
\subsection{Two-Layer Networks}
\label{sec:TwoLayerNetworks}
\begin{table}[htb]
\caption{ Performance of layer-one of a two-layer classifiers. Softmax network at layer-one correctly identified 87.17\% of fall events from non-fall events, but \hl{the} neural network correctly identified all fall and non-fall events.}
...
\par
 Because of \hl{its} 100\% recognition rate we choose neural networks as layer-one network. In the subsequent discussion, unless otherwise mentioned,  layer-one classifier is a neural network with two hidden units.
...
\hl{the performances} of layer-two networks are shown in Tables~\ref{Layer2FallEvents}, \ref{Layer2NonFallEventsSoftmax}, and \ref{Layer2NonFallEventsNeuralNets}. It is easy to see from Table~\ref{Layer2FallEvents} that both Softmax and Neural networks at layer-two correctly classified 100\% of fall events.
...
While \hl{the} evaluation of the proposed techniques using our off-the-shelf device has proven to be effective, the project is far from complete. It requires further testing. Also, for real-world deployment, a miniaturized prototype must be developed and tested on a wide population. We expect that the software platform will require only minimum modifications, if at all.
