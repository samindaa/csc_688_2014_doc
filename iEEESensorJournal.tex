...
\mathbf{x})$. We have 
used the regularized cross-entropy negative-log likelihood function, $l(\mathbf{x}) = 
-\mathbf{y}$.ln 
$\mathbf{z} + \frac{\lambda}{2} \mathbf{W}.\mathbf{W}$ (``.'' represents the scalar dot  
or Frobenius product, with $\lambda$ as the regularization parameter),
as our 
...
used the rectified linear unit  activation function in the hidden layers 
...
%able-bodied 
humans. The list of these activities and fall events are shown in Table~\ref{Tbl:ListOfActivities}. 
The normal activities include walking 
(forward, backward, left, and right), marching, and rotating (left and right). Falling  (forward, 
backward, left, and right) has been considered as an abnormal event.   

 As shown in Fig.~\ref{fig:deviceWithSubjects}, a wireless 9-axis  motion sensing device was attached to the back of the subjects and a wireless data collecting device was connected to a computer. These two devices formed a WSN.
\par
 While a subject performed prescribed activities and simulated-fall events, data was logged in the computer 
for extracting training examples for learning algorithms as well as  performance evaluation.
The motion data included 3-axis accelerometer readings, 3-axis gyroscope readings, and 3-axis magnetometer readings.  Figures \ref{fig:human_fall_forward_crop} and \ref{fig:human_walk_forward-crop} show plots of motion data for\begin{inparaenum}[($i$)] \item fall forward and \item walk forward\end{inparaenum}, respectively.
...
We  trained two one layer networks: softmax regression and neural networks (see Fig.~\ref{fig:SoftmaxLayer1} for block diagrams of these networks). Performance of these networks are discussed in the next two paragraphs.
 ...
 As can be seen from Table~\ref{tab:softmaxOnelayer}, one-layer sofmax regression network with 6 inputs and 11 outputs recognized on an average 94.4\% of all  events. A closer examination reveals that all but fall forward, walk forward, and walk right events are recognized correctly. Recognition rate for fall forward event is only 66.7\%. This event is sometimes  incorrectly classified as walk back or march events. Walk forward event is correctly recognized on an average rate of 87.5\% and is incorrectly recognized as  walk right. Recognition rate for walk right is same as walk forward and is confused with  walk left.
\begin{table}[htb]
\caption{Performance of a Neural network with one row of neurons with sigmoid activation functions and one row of elements with softmax activation function. On an average 95.8\% of the events were classified correctly.}
...
Performance of neural networks is shown in Table~\ref{tbl:neuralNetworkOneLayer}; it recognized 95.8\% of all events correctly. Before we discuss our results, it should be noted that the network is composed of one row of neurons with sigmoid activation function followed by a row of elements with softmax activation function (see Fig.~\ref{fig:SoftmaxLayer1} in Section~\ref{sec:OffLineLearning}). An advantage of this architecture is that the number of neurons in the first row  can be varied for optimal performance.
...
\par
Because of failure of one layer networks, we explored two-layer networks: first layer classified input into two categories, fall or non-fall. Then, a second layer  differentiated events within its own category. A block of the classifier  is shown in Fig.~\ref{fig:TrainedIdentificationModule}.
...
\subsubsection{Layer One: Fall and Non-fall identification}
\label{sec:LayerOneFallAndNonFallIdentification}

Performance of layer-one of a two-layer classifiers is reported in Table~\ref{tbl:layer1RecognitionRates}. As can be seen from the table, softmax network at layer-one (top layer) correctly classified only 87.2\% of fall and non-fall events, but neural network correctly classified all fall and non-fall events. Therefore, we choose neural networks as layer-one network. In the subsequent discussion, unless otherwise mentioned, results reported for layer-two classified utilizing a neural network at layer-one.
...
\subsubsection{Layer Two: Fall or Non-fall Event Identification}
\label{sec:LayerTwoFallOrNonFallEventIdentification}

Performance of layer-two networks are shown in Tables~\ref{Layer2FallEvents},\ref{Layer2FallEvents}, and \ref{Layer2FallEvents}. It is easy to see from Table~\ref{Layer2FallEvents} that both Softmax and Neural networks at layer-two correctly classified 100\% of fall events.
Results for seven non-fall events are shown in Tables~\ref{Layer2FallEvents} and \ref{Layer2FallEvents}. Again both Softmax and Neural networks achieved 100\% accuracy.
\begin{table}[htb]
