...
response in order to prevent the accident. Currently, a plethora of inexpensive wireless motion sensing devices are available or can be assembled using off-the-shelf components. They can be used to collect motion data in 
...
appropriate thresholding. However, the but same method would not be as successful for our human
...
the classification of different events. Bao \& Intille 
...
right ankle, and the right hip  of the subjects for collecting motion data. They 
aim at recognizing 20 activities including walking to riding elevator for
bicycle training. They evaluated the performance of a decision table,
...
They reported 94\% accuracy for fall-detection. Howver, the proposed system  could
...
\par To the best of our knowledge only three studies have been dedicated using
biped robots \cite{Andre2015,Goswami2014,Moya2015}. The focus for these studies
...
floor). Therefore, each fall dataset, there exists a duration in which the fall
...
from each other and all 7 non-fall events. Although, separation of non-fall
events in Fig.~\ref{fig:automatic_annotation3} do not show as good separations,
but our results show that they are far enough from each other for 100\%
accurate classifications.

Since we are interested in the signatures of the fall events, we have annotated
all the samples belongs to ``a'' cluster as the fall data. In general, the
...
comments have improved presentations of the contents.
