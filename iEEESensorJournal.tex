...
While inexpensive wearable motion-sensing devices have shown great promise for fall detection and posture monitoring,  two major problems still exit and have to be solved: \begin{inparaenum}  \item framework for development of firmware, and \item software to make intelligent decisions. \end{inparaenum} We address both the problems. 
%The method used
We propose a generic framework for developing firmware. We also demonstrate that k-means clustering algorithm can semi-automatically extract training examples from motion data. 
...
The proposed classification-networks are combinations of neural
and Softmax regression networks. 
%The results
These networks are trained offline with training examples extracted  by our proposed method. Cross-validations of trained two-level networks show 100\% acuracy for non-fall activities and fall events. The dataset for training and testing have been collected using the devices we assembled with four off-the-shelf components and programmed them using a prototype of our proposed framework. The dataset include seven types of non-fall activities and four types of fall events.
%Conclusion reached
...
\centering
  ...
  \includegraphics[width=0.9\columnwidth,height=1.5in]{plots/human_falling-crop_bw.eps}}}
  \mbox{\subfloat[Motion dataset for walk forward event of a human subject. 
  ]{\label{fig:human_walk_forward-crop} 
  \includegraphics[width=0.95\columnwidth,height = 1.5in]{plots/human_walk-crop_bw.eps}}}
...
appreciate  constructive comments and suggestions of anonymous reviewers and editors. Their
comments have improved presentations of the contents, especially abstract, Sections~\ref{sec:Intro}, and \ref{subSec:relatedWork}.
