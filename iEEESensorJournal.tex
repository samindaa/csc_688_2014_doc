...
\begin{inparaenum}[1)] \item  to setup the WSN, \item to collect data, \item to extract training examples from sensor readings, \item to learn from sample examples, and \item 
...
Section~\ref{sec:SemiAutomaticExtractionOfTrainingVectors} reports our novel methods for  semi-automatic extraction  of \begin{inparaenum} [1)] \item training samples to be used by the learning algorithms  and \item testing data for activity monitoring. \end{inparaenum} Outlines of offline leaning algorithms used for teaching the device to identify activities and fall events are described in Section~\ref{OffLineLearning}. 
...
\subsection{Ordering of Sensor Data Collection and Processing}
...
\subsection{Utilization of FDCP to 9-Axis Motion Sensor}
...
\section{Semi-Automatic Extraction of  Training Example}
...
\subsection{Preprocessing of the Time Series Data}
...
As a preprocessing step, in order to filter the noise in the input vectors, we have 
created a moving window that contains 20 consecutive samples, which amounts to 400$ms$, 
and obtain the 
average value. We have moved the window 200$ms$, which amounts to 10 samples, and 
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.48\textwidth]{figures/example_annotation-crop.pdf} 
