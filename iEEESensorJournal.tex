...
For our case we set $k = 2$ for extracting  training examples for fall events. As shown in Figures~\ref{fig:automatic_annotation} and \ref{fig:automatic_annotation2}.
...
\subsection{Softmax Regration Algorthm}
...
term, $x_0 = 1$, therefore, $\bm{x} \in \mathbb{R}^7$. The model uses a parameter matrix 
$\bm{W} 
\in \mathbb{R}^{7 \times k}$, and the output vector, $\bm{a} = \bm{Wx}$, is passed 
through 
the 
Softmax 
activation function $\bm{z} = \frac{e^{\bm{a}}}{\bm{1}^\mathtt{T} e^{\bm{a}}}$. We have 
