...
Wearable Sensors}
%\author{Saminda Abeyruwan \and Faisal Sikder \and Dilip Sarkar\\
% University of Miami \\
...
such 
as \begin{inparaenum}[(1)] \item InvenSense MPU-9150: 9-axis MEMS motion tracking (thee-axis 
...
MPU9150Module} contains logic to 
read/write from MPU-9150 9-Axis (Gyro+Accelerometer+Compass) MEMS MotionTracking device on the 
...
running and so on, an abnormal event such as a fall may occur. 
% These events may cause injury
% to a human body or to the structural components of a humanoid robot. In order to avoid 
% damages to biped humanoid robots, machine learning can be used to predict potential falls before 
% such events occur. Using the prediction information, corrective measures can be engaged to avoid 
% many fall activities. Similarly, for humans, after  a fall activity is identified, rescue services 
% can be called upon. 
We have reported a wireless sensing device that were assembled using 
