...
\begin{figure*}[!t]
\centering
\subfloat[]{\label{fig:fa}\includegraphics[width=0.6\textwidth]{figures/graph_structure_def-crop}} 
\subfloat[]{\label{fig:b}\includegraphics[width=0.2\textwidth]{attached_device_cropped}}
\subfloat[]{\label{fig:b}\includegraphics[width=0.2\textwidth]{human_figure_sml}} 
\caption{An example of modules and representations available in CSC688 Energia. (a) Tiva C 
microcontroller with sensor bub was attached to the back of a NAO humanoid
robot; and (b) Sensor Hub BoosterPack. The roll is around x-axis, the pitch is around y-axis, and
the yaw is around z-axis.}
 \label{fig:framework}
\end{figure*}

The software development framework\footnote{We anonymize the link the library.} uses a notion of 
``modules'' and ``representations'' to perform
computations. The modules implement functions, while the representations exchange information from
one module to another. Figure \ref{fig:fa} shows an example of modules and representations. 
The boxes represent modules, while the ellipses represent the representations. As an
...
provide multiple representations. The blue arrows shows the provided representations, the black 
arrows show the requested representations, and the red arrows show the used representations.
The end uses of the software only requires to write modules and representations, while the 
framework 
will compute the topologically sorted graph out of the nodes. This will be computed once, and the
...
side.]{\label{fig:tsnedataset_14}\includegraphics[width=0.25\textwidth]
       {plot4_fallen-crop}}      
  \caption{The figures (a)-(d) show the roll and pitch angles (raw and filtered) for normal 
behavior, while, the figures (e)-(h) show the raw and filtered roll and pitch angles for fallen 
states of the NAO humanoid robot.}
  \label{fig:normalFallenBehavior}

\end{figure*}

Our NAO robots run {\em Robocanes} binary, which has been primarily written for soccer
playing. The walking engine of {\em Robocanes} conducts primary movements. These movements are
